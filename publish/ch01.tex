\chapter{構文解析の基本}\label{ch01}
\index{こうぶんかいせき@構文解析!基本}
\index{さんじゅつしき@算術式}

序章では、構文解析の歴史と広がりを外側から眺めました。この章からいよいよ、構文解析そのものの説明を始めます。まずは簡単な算術式の構文解析を例にして、構文解析の基本について学ぶことにしましょう。

算術式という言い方は堅苦しいですが、要は数式のことです。たとえば、\texttt{1\ +\ 2}や\texttt{(1\ +\ 2)\ *\ 3}、\texttt{(3\ *\ 3)\ *\ (2\ *\ 3)}のようなものです。プログラマの皆さんなら普段、日常的にプログラムの一部として算術式を書いているでしょう。算術式は非常にシンプルな「言語」なので、構文解析の題材にするのに最適なのです。
...
皆さんは何かしらのプログラミング言語を使ってプログラムを書いているはずですから、上のような算術式はなじみが深いはずです。

しかし、上のような日本語を使った定義だけでは算術式の文法を表現するのには不十分です。たとえば「式の優先順位を変えるために括弧を使える」といっても、なんとなくはわかるものの、定義としては曖昧です。もちろん、日本語で詳しく記述して曖昧さを少なくしていくこともできますが、いたずらに長くなるだけです。

曖昧さがない形で文法を表現するために、特別な記法を使うことが一般的です。たとえば、皆さんがJavaのメソッドを書くときにも決まった形式があるように（例：\texttt{public\ void\ methodName()\ \{\ ...\ \}}）、プログラミング言語の文法そのものを記述するときにも決まった形式があるのです。

次節では、そのような「特別な記法」の中でも最も広く使われているBNF（Backus-Naur
Form、バッカス・ナウア記法）を使って、算術式の文法を表現してみましょう。
...
BNFは、プログラミング言語の文法をはじめ、インターネット上でのメッセージ交換フォーマットなど、さまざまな文法を表現するのに使われています。本書の読者の方にはBNFになじみのない方も多いと思うので、算術式のBNFの前にBNFについて説明します。本書では、記述を簡潔にするため、ISO/IEC
14977で仕様が策定されたEBNF\footnote{翻訳: ISO/IEC 14977:1996
  Information technology --- Syntactic metalanguage --- Extended BNF
  \url{https://hazm.at/mox/lang/meta-language/ebnf/iso-iec-14977-extended-bnf/index.html}}で用いられる繰り返し（\texttt{\{\}}）、オプション（\texttt{{[}{]}}）、グループ化（\texttt{()}）といった拡張記法を一部取り入れ、これを広義のBNFとして扱います。なお、Java言語仕様など多くの仕様書でも、この種の拡張BNFが一般的に用いられています。BNFには歴史的に多くの方言が存在するため、本書で用いる記法について事前に説明しておきます。

\subsection{BNFの概要}

BNFはFORTRANの開発者でもあるJohn
Backus（ジョン・バッカス）らが開発した記法です。BNFは「プログラミング言語」そのものの文法を記述するために開発されました。かつては基本情報技術者試験でも定番の出題項目だったので、そこで知った方もおられるかもしれません。

本書で用いる広義のBNF（EBNFの要素を取り入れたもの）の主要な記法は以下のとおりです。
...
\item
  ...
  のようにカンマで表すのが標準です）。
\item
  \texttt{\textbar{}} : 選択を表します。たとえば \texttt{A\ \textbar{}\ B}
  は「AまたはB」を意味します。
\item
  \texttt{\{\}} : 0回以上の繰り返しを表します。たとえば \texttt{A\ \{B\}}
  は「Aの後にBが0回以上続く」ことを意味します。
\item
  \texttt{{[}{]}} : 0回または1回の出現（オプション）を表します。たとえば
  \texttt{{[}A{]}\ B}
  は「Aがあるかもしれないし、ないかもしれないが、その後にBが続く」ことを意味します。
\item
  \texttt{()} : グループ化を表します。たとえば
  \texttt{(A\ \textbar{}\ B)\ C}
  は「AまたはBの後にCが続く」ことを意味します。
\item
  \texttt{\textquotesingle{}...\textquotesingle{}} または
  \texttt{"..."} : 終端記号（リテラル文字列）を表します。
\item
  非終端記号（例: \texttt{expr}, \texttt{term}）:
  他の規則で定義される記号を表します（用語の詳細は後述します）。
\end{itemize}

では早速、BNFの具体例を見ていきましょう。以下の例は1.1節で出てきた算術式を元に、
...
たくさんの記号が出てきて戸惑われたかもしれませんが、1つずつ丁寧に見ていきましょう。

\subsection{expr}

BNFでは、以下のような\textgt{生成規則}の集まりによって、文法を表現します。

\begin{codescreen}{}
expr = term { ('+' | '-') term };
\end{codescreen}

\texttt{=}の左側である\texttt{expr}が\textgt{左辺}（この規則で定義される非終端記号）で、右側が\textgt{右辺}（その定義）になります。
...
\noindent が右辺です。本書では、右辺のことを規則の\textgt{本体}とも呼びます。

本体の中に出てくる、他の規則を参照する部分（ここでは\texttt{term}）を\textgt{非終端記号}と呼びます（この用語は前にも登場しましたが、ここで正式に定義します）。これは「まだ展開の余地がある記号」という意味で、同じBNF内で定義されている他の非終端記号と一致する必要があります。Javaでいえば、メソッド呼び出しのようなものです。

一方、\texttt{\textquotesingle{}+\textquotesingle{}}や\texttt{\textquotesingle{}-\textquotesingle{}}のように\texttt{\textquotesingle{}}で囲まれた記号や文字を\textgt{終端記号}と呼びます。これは「これ以上展開できない、最終的な文字」という意味で、実際にプログラムに書かれる文字そのものを表します。Javaでいえば、\texttt{+}演算子や\texttt{;}（セミコロン）のようなものです。
...
なお、ISO/IEC 14977のEBNFでは

\begin{codescreen}{}
expr = term, { ('+' | '-'), term };
\end{codescreen}

\noindent のように要素間をカンマで区切りますが、本書では可読性を考慮してスペースで区切るようにしています。
...
この規則は、まず\texttt{factor}が出現し、その後に「\texttt{*}または\texttt{/}とそれに続く\texttt{factor}」が0回以上繰り返して出現するものが\texttt{term}であることを示しています。
...
\section{BNFを使った解析プロセス}

算術式の文法を規則\texttt{expr}を含むBNFによって表現することができました。この記述には\textgt{曖昧さがなく}、スペースを許さないことや演算子の優先順位などの概念も含まれています。

この規則\texttt{expr}を使うと、式\texttt{1+2}はどのように解析できるのでしょうか？ここでは、その概要について簡単に説明します。

このBNFを使って解析を行う方法は一意ではなく、さまざまなやり方（手続き）があります。その個々の手続きがまさに構文解析アルゴリズムとなります。個々の構文解析アルゴリズムについては第4章で詳しく説明しますが、ここでは、BNFを使って\textgt{素直に}算術式を解析する方法の概要を説明します。ちなみに、構文解析に限った話ではありませんが、そのような\textgt{素直な}方法のことをナイーブ（naive）な方法と呼ぶことがあります。
...
たとえば、\texttt{expr("1+2")}という関数呼び出しは、文字列\texttt{"1+2"}を解析することを示します。まだ\texttt{expr}の本体がありませんが、結果としては、解析が成功した旨の情報が返ってきてほしいでしょう。一方、\texttt{expr("1+")}という関数呼び出しでは、解析が失敗した旨の情報が返ってきてほしいはずです。
...
\section{なぜこのBNFで演算子の優先順位を表現できるのか}
\index{えんざんし@演算子}
\index{ゆうせんじゅんい@優先順位}
\index{けつごうせい@結合性}
\index{ぶんぽうせっけい@文法設計}
...
その答えは、\textgt{文法の階層構造}にあります。BNFを改めて見てみましょう。
...
\begin{enumerate}
\item
 \texttt{expr}が\texttt{factor}を呼び出す
\item
...
このような階層構造により、パーサーは自然に演算子の優先順位を守りながら解析を進められるのです。
...
皆さんおなじみのJSONがそうであるように、実用的には解析した結果を何らかのデータ構造に変換して格納しておく必要があります。そのためのデータ構造が\textgt{抽象構文木}（Abstract Syntax Tree）です。

抽象構文木とは何でしょうか？今、私たちがほしいのは
...
\noindent ような構造です。抽象構文木は、まさにそのようなニーズを満たすデータ構造です。
...
抽象構文木（AST）では、各ノードはプログラムの構成要素を表し、親子関係によって演算子とその演算対象（オペランド）の関係性を示します。たとえば、\texttt{1\ +\ 2}という式では、\texttt{+}が演算子、\texttt{1}と\texttt{2}がオペランドです。抽象構文木には次のようなノードがあります。

\subsection{内部ノード}

演算子や制御構造など、他のノードを子として持つノードです。
先ほどの例でいうと、\texttt{+}ノードと\texttt{*}ノードが内部ノードです。

BNFの観点から言えば、内部ノードは主に生成規則の右辺における記号の2つ以上の並びに対応します。

\subsection{葉ノード}

子を持たないノードです。たとえば、\texttt{1}や\texttt{2}、\texttt{3}といった数値は葉ノードとなります。
...
抽象構文木の最上位（根）に位置する、プログラム全体を表すノードです。数式の例でいえば、全体を表す\texttt{+}ノードが根ノードとなります。
内部ノードと根ノードは排他でないことに注意してください。たとえば、\texttt{+}ノードは内部ノードであり、同時に根ノードでもあります。

\subsection{優先順位}

改めて、先ほどの抽象構文木を見てみましょう。
...
この木構造では、\texttt{+}が根ノードであり、その左の子が\texttt{1}、右の子が\texttt{*}です。\texttt{*}ノードの子として\texttt{2}と\texttt{3}が配置されています。これにより、演算の優先順位が明確に表現されるわけです。

この抽象構文木を見れば、\texttt{1\ +\ (2\ *\ 3)}
という演算順序が表現されており、\texttt{(1\ +\ 2)\ *\ 3}
のような異なる解釈にはならないことが直感的に理解できるでしょう。

\subsection{抽象構文木をJavaで表現する}
...
\texttt{Add}、\texttt{Sub}、\texttt{Mul}、\texttt{Div}、\texttt{Num}は、それぞれ加算、減算、乗算、除算、数値を表すレコードです。これによって、抽象構文木をJavaで表現できます。
...
各クラスのインスタンスを作成することで、抽象構文木を表現できます。

\subsection{抽象構文木を評価する}

構文解析が完了して抽象構文木が得られると、その先にはどのような可能性が広がるのでしょうか。実は、抽象構文木さえ作ってしまえば、それを活用してさまざまなことができるようになります。その一例として、ここでは抽象構文木を「評価」して計算結果を得る方法を紹介します。

抽象構文木を作成するだけでは、プログラムの実行結果を得ることはできません。抽象構文木を評価するためには、再帰的な処理を行うことが必要です。抽象構文木をたどりながら、各ノードの演算を行うことで、プログラムの実行結果が得られます。
...
このように抽象構文木を再帰的にたどることで、式の評価が実現されます。

\texttt{Div}
の場合のゼロ除算エラー処理は、ゼロ除算を明示的に検出するために不可欠です。他の演算（\texttt{Add},
\texttt{Sub},
...
\texttt{eval}メソッドを使うことで、次のように抽象構文木を評価できます。
...
抽象構文木をデータとして表現することで、プログラムの構造を簡単に解析できるのです。

\section{まとめ}

この章では、算術式の文法を例題としてBNFについて紹介し、BNFに基づいて算術式を解析する方法の概要について説明しました。また、抽象構文木についても紹介し、抽象構文木をJavaで表現する方法と、抽象構文木を評価する方法について説明しました。

しかし、今のままではBNFに基づく「構文解析器」は与えられた文字列が文法にマッチするかどうかを判定するだけで、抽象構文木を生成できません。次章では、ここで学んだBNFの考え方を応用して、実際にJavaで動作するJSONの構文解析器を実装し、JSONの抽象構文木を生成する方法について詳しく説明します。
