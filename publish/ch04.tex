...
本章は構文解析の主要アルゴリズムを網羅的に扱うため、内容がやや重くなっています。特にLR系（LR(0)、SLR(1)、LR(1)、LALR(1)）は、概念の積み重ねが多く、初読でつまずく読者が一定数います。そこで、本章をどのように読むかについて、2つのおすすめルートを示します。
...
そんなときに、該当する節だけ読み返して理解できるように、各節を可能な限り独立した「リファレンス」として書いています。初読ですべてを理解できなくても、まったく問題ありません。
...
\begin{itemize}
\item
  \texttt{P\ →\ (\ P\ )\ P} ：\texttt{(}で始まる
\item
  \texttt{P\ →\ ε} ：空文字列（何もない）
\end{itemize}

ここで、どちらを選ぶかを事前に決める必要があります。ここでは、先読み文字が\texttt{(}のときは前者、それ以外のときは後者を選ぶこととします。

というわけで、先読み文字が\texttt{(}なので、\texttt{P\ →\ (\ P\ )\ P}を選択します。この規則をスタックに追加します。
...
次の先読み文字は \texttt{)} です。今度は、\texttt{P\ →\ ε}
...
次の先読み文字は \texttt{)} です。ここで、\texttt{P\ →\ ε}
を選択します。空文字列にマッチするので、ドットを進めるだけで、入力からは何も消費しません。
...
\item
  入力をスタックに積んでいく（シフト操作）
\item
  スタックの中身を見て、文法規則にマッチするパターンがあれば還元操作を行う
\item
  還元とシフトを繰り返し、最終的に開始記号だけが残れば成功
\end{enumerate}
...
\item
  スタックの末尾\texttt{(,\ )}が規則\texttt{X\ →\ ()}の右辺と一致。2つの記号を\texttt{X}に\textgt{還元}
...
\item
  スタックの末尾\texttt{X}が規則\texttt{P\ →\ X}の右辺と一致。1つの記号を\texttt{P}に\textgt{還元}（この実装はシフトする前に、可能な還元をすべて試みます）
...
\item
  スタックの末尾\texttt{(,\ P,\ )}が規則\texttt{X\ →\ (\ P\ )}の右辺と一致。3つの記号を\texttt{X}に\textgt{還元}
...
\item
  スタックの末尾\texttt{X}が規則\texttt{P\ →\ X}の右辺と一致。1つの記号を\texttt{P}に\textgt{還元}
...
\item
  スタックの末尾\texttt{\$,\ P,\ \$}が規則\texttt{D\ →\ \$\ P\ \$}の右辺と一致。3つの記号を\texttt{D}に\textgt{還元}
...
\item
  単純な還元ルール：
  すべての規則を順番にチェックし、最初にマッチしたものを使う。なお、この実装では規則\texttt{P\ →\ P\ X}を\texttt{P\ →\ X}より先に並べていることが重要です。順序を入れ替えると、たとえば\texttt{()()}の解析でスタックが\texttt{{[}\$,\ P,\ X{]}}となった時点で\texttt{P\ →\ X}による還元が先に選ばれ、\texttt{{[}\$,\ P,\ P{]}}というどの規則にもマッチしない形に陥って失敗します。「どの還元を選ぶか」の判断が本質的に難しいというこの弱さも、後述するLR法が解決する問題の1つです
\item
  明示的なスタック操作：
...
しかし、すべての構文がこのように単純ではありません。たとえば、以下の単純な算術式を考えてみましょう。\texttt{N}は数値を表す終端記号とします。
...
上の算術式の例のように、1つの非終端記号が複数のトークンで始まる場合があります。これを体系的に扱うために\textbf{\sffamily FIRST}\textgt{集合}（その非終端記号から始まる可能性のあるトークンの集合）が必要です。
...
else節は「あってもなくてもいい」要素です。このような場合、else節の後に何がくる「可能性がある」かを知る必要があります。これが\textbf{\sffamily FOLLOW}\textgt{集合}（その非終端記号の後にくる可能性のあるトークンの集合）です。
\end{enumerate}
...
ところで、なぜ\texttt{nullable}が重要かというと、\textbf{\sffamily FIRST}\textgt{集合}および\textbf{\sffamily FOLLOW}\textgt{集合}を計算するアルゴリズムで必要になるためです。\textbf{\sffamily FIRST}\textgt{集合}の計算では、非終端記号が空文字列を生成できるかどうかを確認する必要があります。\textbf{\sffamily FOLLOW}\textgt{集合}の計算でも、非終端記号が空文字列を生成できる場合、その後にくるトークンを正しく扱うために\texttt{nullable}が必要です。
...
\begin{enumerate}
\item
  すべての非終端記号Eについて\texttt{nullable(E)\ =\ false}に初期化
\item
...
\begin{itemize}
\item 先読みトークン\texttt{a}が\texttt{FIRST(α)}に含まれている場合、この規則を適用する
\end{itemize}
...
先ほど「シフト還元構文解析のJava実装」（4.7節）で示した\texttt{DyckShiftReduce}の実装を思い出してください。
...
たとえば、Dyck言語の解析で考えてみましょう。文法は、「例で学ぶシフト還元構文解析」（4.6.3節）で導入したε規則なしの以下の文法を引き続き使います。
...
\textgt{今どの状態にいるか}がわかれば、\textgt{次の行動が決まる}のです。
...
\item
  ...
  \item
    次は\texttt{P}を読みたい
  \item
    \texttt{P}は1個以上の\texttt{X}の並び（中身が空のケースは
    \texttt{X\ →\ ()}が受け持つ）
  \end{itemize}
\item
...
この状態遷移は決定的で、事前にすべて計算できます。第3章で学んだことを思い出してください。このような状態遷移はオートマトンとして表現できるのです。
...
前節ではシフト還元構文解析のために\texttt{\$}や\texttt{X}を加えて拡張した文法（拡張文法）を使いましたが、ここでは状態の見通しを優先して元の簡潔な文法に戻しています（拡張文法でも同様にオートマトンを作れますが、状態数が増えて図が煩雑になるためです）。
...
\draw (I3) edge[loop below] node[below] {P} ();
...
拡張開始記号自体は本質的ではありませんが、実用上は重要なので覚えておくとよいでしょう。
...
LR(0)項集合を構築する上で最も重要なのが、LR(0)項の\textgt{閉包}（Closure）です。これは、あるLR(0)項から「論理的に導かれるすべての項」を表す概念です。
...
このように、ドットの直後に非終端記号がある場合、その非終端記号が左辺にあるすべての規則を追加していく必要があります。
...
を積み、1つ前の状態（\(I_0\) か \(I_5\)）から \texttt{T}
での遷移をたどって \(I_2\)
...
\texttt{S\textquotesingle{}\ →\ S\ \$・}
に到達した受理状態であることを示しています。実行時にはすでに\texttt{\$}を読み終えていますが、表では「この状態にきたら受理する」という印として便宜上\texttt{\$}列に\texttt{acc}を書いています。
...
この文法でLR(0)オートマトンを構築すると、先ほど構築した状態\(I_2\)が以下の項を含むことになります。
...
LR(1) へ）。\enlargethispage{\baselineskip}
\end{quote}
...
\begin{quotation}
  「ある非終端記号Aの後にきうる記号」がわかれば、還元すべきタイミングがわかる。
\end{quotation}
...
LR(0)では「還元可能な状態ではすべての入力に対して還元」していましたが、SLR(1)では「その非終端記号の後に実際にくる可能性がある記号（FOLLOW集合）の時だけ還元」します。これにより、多くのコンフリクトが解決されます。
...
この方法は概念的にわかりやすいですが、一時的に大きなLR(1)オートマトンを構築する必要があります。
...
この状態で、先読みがdまたはeのとき、AとBのどちらで還元すべきか決定できません（reduce/reduceコンフリクト）。
...
LR(1) & 最強（あらゆる決定的文脈自由言語） & 数百〜数千以上に膨らむ & 主に理論研究。ただしPager法などで状態数を縮約した実用実装（OCamlのMenhirなど）もある & 項目自体に先読み記号を持たせる\\ \hline
...
Grammar（PEG）}は、全く違うアプローチを取ります。これまでの構文解析手法は大前提として、文脈自由文法を元にしていましたが、PEGは文脈自由文法に一見よく似た、それでいて異なる新たな形式文法です。異なる形式文法であるため、CFGと表現能力は異なります。
...
PEGパーサーにもメモ化を適用できます。単にすべての規則に対してメモ化を行ったものを\textbf{\sffamily Packrat
...
LL(1) & \(O(n)\) & \(O(|N| \times |T|)\) & \(|N|\):非終端記号数,
\(|T|\):終端記号数\\ \hline
LL(k) & \(O(n)\) & \(O(|N| \times |T|^k)\) &
...
Packrat & \(O(n)\) & \(O(n \times |P|)\) & \(|P|\): 規則数\\ \hline
    \end{tabular}
  \end{center}
\end{table}

表の「O表記」は、アルゴリズムの計算量を表す記法で、\(O(n)\)は「入力の長さに比例した時間」、\(O(n^2)\)は「入力の長さの2乗に比例した時間」を意味します。なお、空間計算量の欄は、LL系・LR系では事前に構築する解析表のサイズを、PEGとPackrat Parsingでは解析時に使用するメモリ量を示しています。
...
構文解析は難しく感じるかもしれませんが、基本的なアイデアは「文字列を解析して木構造を抽出する」というシンプルなものです。この章で学んだ各手法の特徴を理解しておけば、実際の開発で構文解析が必要になったときに、適切な選択ができるはずです。
