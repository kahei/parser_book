...
\item
  単純な還元ルール：
  すべての規則を順番にチェックし、最初にマッチしたものを使う。なお、この実装では規則\texttt{P\ →\ P\ X}を\texttt{P\ →\ X}より先に並べていることが重要です。順序を入れ替えると、たとえば\texttt{()()}の解析でスタックが\texttt{{[}\$,\ P,\ X{]}}となった時点で\texttt{P\ →\ X}による還元が先に選ばれ、\texttt{{[}\$,\ P,\ P{]}}というどの規則にもマッチしない形に陥って失敗します。「どの還元を選ぶか」の判断が本質的に難しいというこの弱さも、後述するLR法が解決する問題の一つです
\item
  明示的なスタック操作：
