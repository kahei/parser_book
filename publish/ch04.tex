...
非終端記号の後に現れうるトークンの集合を\textbf{\sffamily FOLLOW}\textgt{集合}と呼びます。これは、\textbf{\sffamily nullable}な非終端記号（空文字列規則を持つ非終端記号）を扱うために特に重要です。
...
\begin{enumerate}
  \item \verb|FOLLOW(E) = {$}|（開始記号として）
  \item 規則 \texttt{E\ →\ (\ E\ )} から
  \begin{itemize}
  \item Eの後に)が来るので、)をFOLLOW(E)に追加
  \item \verb|FOLLOW(E) = {$, ')'}|
  \end{itemize}
  \item 規則 \texttt{E\ →\ -\ E} から
  \begin{itemize}
  \item Eが末尾にあるので、\verb|FOLLOW(E)|を\verb|FOLLOW(E)|に追加（変化なし）
  \end{itemize}
  \item 規則 \texttt{E\ →\ (\ E\ )} から（2回目）
  \begin{itemize}
  \item すでに ) は追加済みなので、変化なし
\end{itemize}
  \item 規則 \texttt{E\ →\ -\ E} から（2回目）
  \begin{itemize}
  \item 変化なし。すべての規則を見て変化がなくなったので終了
\end{itemize}
\end{enumerate}
...
\textbf{\sffamily 2. FOLLOW}\textgt{集合による判断} --- 規則 \texttt{A\ →\ ε}（空文字列規則）について
...
\item
  \textbf{\sffamily 1982}\textgt{年}：DeRemer \& Pennello, ``Efficient Computation of LALR(1) Look-Ahead Sets''（TOPLAS）でLALR(1)の効率的構築アルゴリズムが確立され、Yacc/Bisonなど実用システムに広く採用されました。
\end{itemize}
