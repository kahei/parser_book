...
たとえば、Javaで電話番号の形式を簡易チェックする時、以下のような正規表現を使うことがあります：
...
\subsubsection{連接（Concatenation）}\label{chapter4-ux9023ux63a5concatenation}}
...
\subsubsection{選択（Alternation）}\label{chapter4-ux9078ux629ealternation}}
...
\subsubsection{繰り返し（Kleene Star）}\label{chapter4-ux7e70ux308aux8fd4ux3057kleene-star}}
...
\path (q1) edge[bend right] node[below] {ボ/商} (q0);
...
\tightlist
\item
  入力\verb|"a"|：状態0→状態1（\textbf{受理}）
\item
  入力\verb|"ab"|：状態0→状態1→状態1（\textbf{受理}）
\item
  入力\verb|"abb"|：状態0→状態1→状態1→状態1（\textbf{受理}）
\item
  入力\verb|"b"|：状態0で\verb|"b"|を読めない（\textbf{非受理}）
\end{itemize}
...
この規則では、Aはどこに現れても常に\texttt{abc}に置き換えることができます。周りの文脈（前後の文字）に関係なく適用できるので「文脈自由」です。自由というと一見「好きにできる」という印象を受けますが、ここの「自由」は、たとえば、アルコールフリー＝アルコールが含まれない、というのと意味での「自由」です。形式言語を扱う研究者の中にはContext-free Grammarを「文脈自由文法」と訳したのは誤解を招きやすいので、あまりよくなかったのではないか、という人もいます。
